...
- **Web Speech API on iOS Safari**: partially checked, still not confirmed. No real device or simulator was available, so this was tested against Playwright's WebKit build (the actual browser engine iOS Safari uses) with an iOS Safari user agent instead. Result: `webkitSpeechRecognition` genuinely exists as a constructor (corrects an earlier assumption that Safari lacks the API entirely) and `.start()` doesn't throw — but produced zero events (no `start`/`result`/`error`/`end`) within 3s in this headless, mic-less sandbox, which is inconclusive rather than a real pass or fail. The app doesn't depend on this resolving either way: it never auto-advances on a recognition event, only reads whatever transcript exists (real or empty) when the student taps to stop, so silence from the API already falls through cleanly to the placeholder transcript. Still worth a real-device check before this is called done.
